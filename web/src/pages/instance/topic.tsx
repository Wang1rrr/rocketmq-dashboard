...
const TopicPage = () => {
  ...
  const handleImportTopics = async () => {
    ...
    if (createdTopics.length > 0) {
      ...
      // The optimistic insert above leaves `totalTopics` and the pager untouched, so the
      // header count stays stale and the new rows can be sliced off the current page.
      // Reload the first page to bring the count, the pager and the rows back in sync.
      if (tablePage === 1) {
